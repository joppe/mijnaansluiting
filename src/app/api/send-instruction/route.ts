...
import { exists } from "@/token/tokens";
import { validateOpdrachtId } from "@/validation/validateOpdrachtId";

export const dynamic = "force-dynamic"; // defaults to auto

function createInstruction(id: string) {
  return {
    opdrachtId: id,
    versienummer: "2.0",
    discipline: "Gas",
...
export async function GET(request: Request) {
  const opdrachtId = new URL(request.url).searchParams.get("opdrachtId");

  if (opdrachtId === null) {
    return Response.json({ error: "No opdrachtId provided" }, { status: 400 });
  }

  if (!validateOpdrachtId(opdrachtId)) {
    return Response.json({ error: "Invalid opdrachtId" }, { status: 400 });
  }

  const instruction = createInstruction(opdrachtId);
